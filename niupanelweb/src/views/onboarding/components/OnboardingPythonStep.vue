<template>
  <div class="flex flex-col gap-6">
    <div class="flex items-center gap-4">
      <div class="h-11 w-11 shrink-0 rounded-lg bg-amber-500/10 text-amber-600 flex-center dark:text-amber-300">
        <div class="i-carbon-logo-python text-xl"></div>
      </div>
      <div>
        <h2 class="text-lg font-bold text-default">创建 Python 虚拟环境</h2>
        <p class="text-xs text-muted mt-0.5">
          所有 Python 脚本将在此独立沙盒中运行，与系统环境隔离
        </p>
      </div>
    </div>
    ...
    >
      <div class="flex items-center gap-2 mb-3">
        <div class="text-xs font-bold text-default">Python 版本</div>
        <div class="ml-auto text-[11px] text-muted">推荐 3.11</div>
      </div>
      <el-input
        v-model="version"
        placeholder="例如: 3.11"
    ...
    >
      ...
      <div class="flex-1 min-w-0">
        <div
          ...
          "
        >
          {{ done ? "Python 环境创建任务已提交 ✓" : "正在创建 Python 环境..." }}
        </div>
    ...
    <div class="flex gap-3">
      ...
      >
        下一步
        <div class="i-ep-arrow-right ml-1"></div>
      </el-button>
    </div>
  </div>
</template>
