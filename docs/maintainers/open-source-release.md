...
- 推送 `v<core-version>` Tag，或手动运行 `Publish NiuPanel Release`，都会生成 GitHub Release。手动运行始终从 `main` 的固定提交构建，并要求输入与 Core 版本一致的 Tag。Release 包含三架构 Core 包、Web 包和 `niupanel-release.json`，后者绑定 Core/Web 的校验和与 Git SHA。
