<template>
  <div class="app-viewport flex flex-col overflow-x-hidden overflow-y-auto bg-base">
    <header class="safe-top-header flex shrink-0 items-center border-b border-light bg-card px-4 pt-safe sm:px-6">
      <div class="flex items-center gap-2.5">
        ...
        <div>
          ...
          <div class="text-[11px] text-muted">首次配置</div>
        </div>
      </div>
    </header>

    <main class="flex flex-1 items-start justify-center px-3 py-4 sm:items-center sm:px-4 sm:py-8">
      <div class="w-full max-w-[640px]">

        <OnboardingStepProgress :step="step" :steps="steps" />

        <div
            class="rounded-lg border border-light bg-card p-4 shadow-sm sm:p-8"
        >
        ...
        <div v-if="step < 3" class="mt-4 text-center">
          <button type="button" class="mobile-touch-target cursor-pointer px-3 text-xs text-muted transition-colors hover:text-default" @click="skipOnboarding">
            跳过初始化，直接进入面板
          </button>
        </div>
      </div>
